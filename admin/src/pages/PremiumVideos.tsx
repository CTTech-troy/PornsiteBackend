...
import { fetchPremiumVideos, updateVideoStatus, deleteVideo, type Video } from '../api/contentApi';
...
export function PremiumVideos() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const res = await fetchPremiumVideos({ page, limit, search, statusFilter });
  ...
  const columns: Column<Video>[] = [
    ...
    { key: 'price', header: 'Price (🪙)', render: (v) => <span className="text-[13px] font-medium text-text-primary tabular-nums">{v.price ? v.price.toLocaleString() : '—'}</span> },
    { key: 'earnings', header: 'Earnings (₦)', render: (v) => <span className="text-[13px] font-medium text-text-primary tabular-nums">₦{Number(v.earnings).toLocaleString()}</span> },
    {
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-xl font-semibold text-text-primary">Premium Videos</h1>
          <p className="text-[13px] text-text-tertiary mt-0.5">{total.toLocaleString()} total premium videos</p>
        </div>
      ...
      <div className="card overflow-hidden">
        <DataTable columns={columns} data={videos} isLoading={loading} emptyMessage="No premium videos found." />
        <Pagination
          currentPage={page}
